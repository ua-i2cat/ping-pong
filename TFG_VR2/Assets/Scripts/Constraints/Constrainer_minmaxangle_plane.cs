...
public class Constrainer_minmaxangle_plane : Constrainer
{
    public bool active;
    ...
    private void ConstrainToAngle()
    {
        ...
        if (axis == Vector3.zero)
        {
            ...
            ToParent = (parent.position - transform.position).normalized;
            ToChild = (child.position - transform.position).normalized;
            axis = Vector3.Cross(ToParent, ToChild).normalized;
            Debug.Assert(axis != Vector3.zero);
            transform.Rotate(-2, 0, 0);
        }
